...
@app.route('/google')
def google():
    return render_template("result.html", title_list = scrape("google") , pageTitle = "Popular Google News")

@app.route('/facebook')
def facebook():

    return render_template("result.html", title_list = scrape("facebook") , pageTitle = "Popular Facebook News")
@app.route('/<search>')
def search(search):
